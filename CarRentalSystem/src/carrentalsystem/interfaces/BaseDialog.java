 ...
 *
 * Use this for: BookingFormDialog — date picker + price calculator
 * ManageBookingDialog — cancel / re-request booking EditListingDialog — edit
 * existing car listing DeleteConfirmDialog — 'Are you sure?' overlays
...
public abstract class BaseDialog extends JDialog {
    /** true only if the user clicked the primary action button (not just closed). */
    protected boolean confirmed = false;
 
    /**
     * Constructor.
     * @param parent  The JFrame or JPanel's ancestor frame. Pass:
     *                (Frame) SwingUtilities.getWindowAncestor(this)
     *                when calling from inside a JPanel.
     ...
     *
     * Return true  → inputs are valid, proceed to onConfirm().
     * Return false → inputs are invalid, show error, block the action.
     *
     * Example (BookingFormDialog):
     *   if (startDate == null) { lblError.setText('Pick a start date'); return false; }
     *   if (endDate.before(startDate)) { lblError.setText('End must be after start'); return false; }
     *   return true;
     */
